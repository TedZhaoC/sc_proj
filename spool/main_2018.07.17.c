...
void* read_raw_sensor(void* port_name)
{
	...
	if(ret != 0)
    {
		printf("TOR_CMD_CONFIGURE_INPUTS going wrong! " NL);
...
//
		...
		//Ted test on July 17th, 2018
		multi_canbus = 9.999; //Ted mark here
		...
		acc_canbus = 66666; //Ted mark here
		...
		//Ted test on July 17th, 2018
		acc_canbus = 55555; //Ted mark here
		...
		//Ted test on July 17th, 2018
		acc_canbus = 44444; //Ted mark here
		...
		//Ted test on July 17th, 2018
		lc_value_sc = 33333; //Ted mark here
		...
		lc_value_sc = 22222; //Ted mark here

		frame.data[4] = ((sc_mode & 0x3) << 3) | (active_lc_num & 0x7);
		frame.data[5] = lc_value_sc & 0xff; // first lc status
		frame.data[6] = lc_value_sc & 0xff; // second
		frame.data[7] = lc_value_sc & 0xff;

		send_port(&frame);//frame-4, uncompensated weight+sc_mode+active number+loacell status [1-3] etc

		//Ted test on July 17th, 2018
		lc_value_sc = 11111; //Ted mark here

		frame.data[0] = lc_value_sc & 0xff; 
		frame.data[1] = lc_value_sc & 0xff; //fifth lc status
		frame.data[2] = 0xff;
		frame.data[3] = 0xff;

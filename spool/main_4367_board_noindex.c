...
//	int lcStatus[8];	// status of load cell input
	...
	//Ted modify it according the latest doc on 2018-07-16
	//TODO - it must be updated when reconfigured through blutooth or other ways
	// and one more SendAndWait(...)
	//....
	...
	//ret = SendAndWait(TOR_CMD_CONFIGURE_MODE, mode_config, 1, resp, &respDataLen, 100, 0);
	...
	while(1)
	{
		...
		else 
		{
			...
			else
			{
				...
				printf(">>>>>>>>>>> Ted 2018-07-16 raw resp[3-4-5-6] is : %x %x %x %x\n", resp[3], resp[4], resp[5], resp[6]);
				printf(">>>>>>>>>>> excitation_v is : %d \n", excitation_v);


				printf("Ted 2018-07-16 active_lc_num and sc_mode are: %d %d \n", active_lc_num, sc_mode);
				...
				for(i=0; i<active_lc_num; ++i)
				{
					...
					value = (unsigned char)resp[3+3 + 10 * i + 1];
					value |= (int)(((unsigned char)resp[3+3 + 10 * i + 2]) << 8);
					value |= (int)(((unsigned char)resp[3+3 + 10 * i + 3]) << 16);
				...
				//	printf(" 55555555555 ---> raw count before minus 0x800000 is :  %d \n", value);
					...
					lcStatus[i] = resp[3+3 + 10 * i + 0];

					acc_x_ave = (unsigned char)resp[3+3 + 10 * i + 4];
					acc_x_ave |= (int)(((unsigned char)resp[3+3 + 10 * i + 5]) << 8);
	
					acc_y_ave = (unsigned char)resp[3+3 + 10 * i + 6];
					acc_y_ave |= (int)(((unsigned char)resp[3+3 + 10 * i + 7]) << 8);
	
					acc_z_ave = (unsigned char)resp[3+3 + 10 * i + 8];
					acc_z_ave |= (int)(((unsigned char)resp[3+3 + 10 * i + 9]) << 8);
				...
				else
				{
					...
					for(bi=0; bi<blocksCount; ++bi)
					{
						...
						for (i = 0; i < 6; ++i)
						{
							...
							{
								// accel
								gyroReadings[i] = ((double)value) * 2 / ((double)0x8000u);
...
*/
	...
	*/
	...
	while (1)
	{
		...
		send_port(&frame); //frame-1, weight+angle

		//---------------------------------
		multi_canbus = (int) (multiplicator * 100); //Ted mark here

		//Ted test on July 17th, 2018
		multi_canbus = 9999; //Ted mark here

		frame.data[0] = multi_canbus & 0xff; 
		frame.data[1] = (multi_canbus >> 8 ) & 0xff; 
		frame.data[2] = (multi_canbus >> 16 ) & 0xff;
		frame.data[3] = (multi_canbus >> 24 ) & 0xff;
		...
		send_port(&frame);//frame-2, multiplicator+accl_x
		...
		frame.data[0] = (acc_canbus) & 0xff;
		frame.data[1] = (acc_canbus >> 8 ) & 0xff;
		frame.data[2] = (acc_canbus >> 16 ) & 0xff;
		frame.data[3] = (acc_canbus >> 24 ) & 0xff;
		...
		send_port(&frame);//frame-3, accl_y+accl_z
		...
		frame.data[0] = lc_value_sc & 0xff; 
		frame.data[1] = (lc_value_sc >> 8 ) & 0xff; 
		frame.data[2] = (lc_value_sc >> 16 ) & 0xff;
		frame.data[3] = (lc_value_sc >> 24 ) & 0xff;

		//Ted mark need double check this mix value

		printf("ted test data[4] is %d %d \n\n\n",  sc_mode, active_lc_num);
		
		frame.data[4] = ((sc_mode & 0x3) << 3) | (active_lc_num & 0x7);
		frame.data[5] = lcStatus[0] & 0xff;
		frame.data[6] = lcStatus[1] & 0xff;
		frame.data[7] = lcStatus[2] & 0xff;

		send_port(&frame);//frame-4, uncompensated weight+sc_mode+active number+loacell status [1-3] etc

		//---------------------------------
		frame.data[0] = lcStatus[3] & 0xff; 
		frame.data[1] = lcStatus[4] & 0xff; 
		frame.data[2] = 0xff;
		frame.data[3] = 0xff;
		...
		send_port(&frame);//frame-5, loacell status [4-5]
